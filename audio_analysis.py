"""
audio_analysis.py — DJ DARK MARK Elite Audio Engine v2.1 + v7 MERGE
=====================================================================
MERGE: Elite v2.1 (snare/hihat/seções/perfil/curva de energia) + v7 (seguro GitHub Actions).

NOVIDADES v2.1 integradas:
...
- librosa e scipy são OPCIONAIS — fallback automático via numpy puro
- numpy é obrigatório (está no requirements.txt)
- Se librosa não estiver disponível, análise básica via FFmpeg subprocess
- Sem travamento, sem OOM, sem crash silencioso
"""
...
# Imports opcionais — GitHub Actions free não garante librosa/scipy
try:
    import numpy as np
    NUMPY_OK = True
except ImportError:
    NUMPY_OK = False
    logger.warning("numpy não disponível — análise muito limitada.")

try:
    import librosa
    LIBROSA_OK = True and NUMPY_OK
except ImportError:
    LIBROSA_OK = False
    logger.warning("librosa não disponível — usando análise FFmpeg básica.")

try:
    from scipy.signal import butter, lfilter
    from scipy.ndimage import uniform_filter1d
    SCIPY_OK = True and LIBROSA_OK
except ImportError:
    SCIPY_OK = False
    if LIBROSA_OK:
...
def _clean_audio(y):
    """Corrige NaN/inf e normaliza o áudio para evitar erros FFmpeg/librosa."""
    if not NUMPY_OK:
        return y
    try:
...
def _duration_ffprobe(path: str) -> float:
    """Duração via ffprobe — sempre disponível no GitHub Actions."""
    cmd = [
        "ffprobe", "-v", "error",
    ...
    try:
        return float(subprocess.check_output(cmd, text=True, timeout=60).strip())
    except Exception:
        return 0.0


def _run_ffmpeg_decode(path: str, sr: int = 22050):
    """Decodifica áudio via FFmpeg para numpy array."""
    if not NUMPY_OK:
        return None, sr
    cmd = [
    ...
    try:
        ...
        return _clean_audio(audio), sr
    except Exception as e:
        logger.warning(f"_run_ffmpeg_decode: {e}")
        return None, sr


# ══════════════════════════════════════════════════════════════════
# FILTROS DE FREQUÊNCIA (requer scipy)
# ══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════
# DETECÇÃO BÁSICA VIA NUMPY (fallback quando librosa não está disponível)
# ══════════════════════════════════════════════════════════════════

def _detect_beats_numpy(y, sr: int = 22050) -> tuple:
    """
    BPM e beats via energia RMS — fallback simples sem librosa.
    Boa o suficiente para beats sintéticos (phonk/trap/eletrônico).
    """
    if not NUMPY_OK or y is None or len(y) == 0:
        return 120.0, []
    try:
        hop = 512
        frames = [y[i:i+hop] for i in range(0, len(y)-hop, hop)]
        rms = np.array([np.sqrt(np.mean(f**2)) if len(f) > 0 else 0.0 for f in frames])
        # Suavização simples
        kernel = min(11, len(rms)//4 | 1)
        if kernel > 1:
        ...
        beats = []
        min_gap = int(0.3 * sr / hop)  # mín 300ms entre beats
        last_beat = -min_gap
        for i in range(1, len(rms_smooth)-1):
            if (rms_smooth[i] > threshold and
                rms_smooth[i] >= rms_smooth[i-1] and
                rms_smooth[i] >= rms_smooth[i+1] and
                i - last_beat >= min_gap):
                beats.append(round(i * hop / sr, 4))
                last_beat = i
        # Estima BPM pelos intervalos
        if len(beats) > 4:
            intervals = [beats[i+1]-beats[i] for i in range(len(beats)-1) if beats[i+1]-beats[i] > 0]
            if intervals:
                med_interval = sorted(intervals)[len(intervals)//2]
                bpm = round(60.0 / med_interval, 1) if med_interval > 0 else 120.0
        ...
        logger.warning(f"_detect_beats_numpy: {e}")
        return 120.0, []


def _detect_drop_numpy(y, sr: int, duration: float) -> Optional[float]:
    """Localiza drop via pico de energia RMS — fallback sem librosa."""
    if not NUMPY_OK or y is None or len(y) == 0:
        return None
    try:
        hop = 512
        ...
        if len(zone) == 0:
            return None
        # Suavização
        kernel = min(60, len(zone)//4 | 1)
        smooth = np.convolve(zone, np.ones(kernel)/kernel, mode='same') if kernel > 1 else zone
        peak = int(np.argmax(smooth)) + s
        return round(peak * hop / sr, 2)
    except Exception as e:
        logger.warning(f"_detect_drop_numpy: {e}")
        return None


# ══════════════════════════════════════════════════════════════════
# DETECÇÃO COMPLETA VIA LIBROSA (quando disponível)
# ══════════════════════════════════════════════════════════════════
...
def detect_beats(y, sr: int) -> tuple:
    ...
    if LIBROSA_OK:
        try:
            ...
            logger.warning(f"detect_beats (librosa): {e}")
    # Fallback numpy
    return _detect_beats_numpy(y, sr)


def detect_bass_hits(y, sr: int) -> list:
    """
    Kicks / 808 / sub-bass: 20-220 Hz.
    Com librosa: análise por frequência real.
    Sem librosa: usa os beats como proxy de kick.
    """
    if LIBROSA_OK:
        try:
            y = _clean_audio(y)
            ...
            env = librosa.onset.onset_strength(
                y=y_b, sr=sr, hop_length=HOP_LENGTH, aggregate=np.median,
            )
            ...
            frames = librosa.onset.onset_detect(
                ...
                pre_avg=4, post_avg=6, delta=0.055, wait=3,
            )
            times = [float(t) for t in librosa.frames_to_time(frames, sr=sr, hop_length=HOP_LENGTH)]
            if len(times) < 8:
                bpm, beats = detect_beats(y, sr)
            ...
            logger.warning(f"detect_bass_hits (librosa): {e}")
    # Fallback: usa beats como proxy
    bpm, beats = _detect_beats_numpy(y, sr)
    return [float(t) for t in beats[:90]]


def detect_snare_hits(y, sr: int) -> list:
    """Snare/caixa: 200-8000 Hz. Retorna lista vazia se librosa não disponível."""
    if not LIBROSA_OK:
        return []
    try:
        y = _clean_audio(y)
...
def detect_hihat_hits(y, sr: int) -> list:
    """Hi-hat/chimbal: 8000 Hz+. Retorna lista vazia se librosa não disponível."""
    if not LIBROSA_OK:
        return []
    try:
        y = _clean_audio(y)
...
def detect_drop(y, sr: int, duration: float) -> Optional[float]:
    ...
    if LIBROSA_OK:
        try:
            ...
            smooth = (
                uniform_filter1d(rms.astype(float), size=60)
                if SCIPY_OK else rms.astype(float)
            )
            total = len(smooth)
            ...
            logger.warning(f"detect_drop (librosa): {e}")
    # Fallback numpy
    return _detect_drop_numpy(y, sr, duration)
...
def classify_beat_intensities(beats: list, y, sr: int) -> list:
    """Classifica cada beat como 'weak', 'medium' ou 'strong' pela energia RMS."""
...
def classify_song_profile(y, sr: int, bpm: float) -> str:
    """
    Classifica como 'aggressive', 'medium' ou 'chill'.
    Controla a intensidade geral dos efeitos visuais.
    Fallback simples por BPM se librosa não disponível.
    """
    if LIBROSA_OK:
        try:
            y = _clean_audio(y)
...
def compute_energy_curve(y, sr: int, n_points: int = 100) -> list:
    ...
    if NUMPY_OK and y is not None and len(y) > 0:
        try:
            hop = 512
            frames = [y[i:i+hop] for i in range(0, len(y)-hop, hop)]
            rms = np.array([np.sqrt(np.mean(f**2)) if len(f) > 0 else 0.0 for f in frames])
...
def full_analysis(audio_path: str) -> dict:
    """
    Análise completa. Retorna dict com:
    bpm, beats, bass_hits, kicks, snare_hits, hihat_hits,
    drop_time, sections, beat_intensities, song_profile,
    energy_curve, duration.

    Funciona com librosa (completo) ou sem (básico via numpy/FFmpeg).
    """
    result = {
        "bpm":              120.0,
        "beats":            [],
        "bass_hits":        [],
        "kicks":            [],  # alias compatível com Remotion/video_generator
        "snare_hits":       [],
        "hihat_hits":       [],
        "drop_time":        None,
        "sections":         {},
    ...
    if LIBROSA_OK:
        try:
            logger.info(f"  [analysis] Carregando via librosa: {Path(audio_path).name}")
            y, sr = librosa.load(audio_path, mono=True)
            y = _clean_audio(y)
            duration = float(librosa.get_duration(y=y, sr=sr))
            result["duration"] = duration
        except Exception as e:
            logger.warning(f"  [analysis] librosa.load falhou: {e} — tentando FFmpeg decode")
            y, sr = _run_ffmpeg_decode(audio_path, sr=22050)
    else:
        logger.info(f"  [analysis] librosa indisponível — usando FFmpeg decode")
        y, sr = _run_ffmpeg_decode(audio_path, sr=22050)

    if y is None or (NUMPY_OK and len(y) == 0):
        logger.warning("  [analysis] Áudio vazio — retornando defaults")
        return result
    ...
    try:
        ...
        logger.info(
            ...
            f"kicks={len(bass_hits)} | snares={len(result['snare_hits'])} | "
...
def crop_analysis(analysis: dict, start: float, duration: float) -> dict:
    ...
    cropped_bass = _crop(analysis.get("bass_hits") or analysis.get("kicks", []))

    return {
        **analysis,
        "beats":            crop_beats,
        "bass_hits":        cropped_bass,
        "kicks":            cropped_bass,
...
def find_best_window(audio_path: str, target_dur: int) -> tuple:
    ...
    Fallback: ffprobe para duração + estimativa pelo drop.
    """
    duration = _duration_ffprobe(audio_path)
    if duration <= 0:
    ...
    if LIBROSA_OK:
        try:
            ...
            logger.warning(f"find_best_window (librosa): {e}")

    # Fallback simples: tenta pegar o "meio forte" da música
    if NUMPY_OK:
        try:
            y, sr = _run_ffmpeg_decode(audio_path, sr=22050)
            if y is not None and len(y) > 0:
                drop = _detect_drop_numpy(y, sr, duration)
                if drop is not None:
                    ideal = max(0.0, drop - target_dur * 0.22)
                    ideal = min(ideal, duration - target_dur)
                    return round(ideal, 2), float(target_dur)
        except Exception:
            pass

    # Último fallback: começa em ~20% da música
    start = min(duration * 0.20, duration - target_dur)
    return round(max(0.0, start), 2), float(target_dur)
...
def build_shake_expression(
    ...
    """
    Expressões (x, y) de shake com bass impulso, hi-hat micro-jitter e drop mega-shake.
    """
    bass_hits  = analysis.get("bass_hits") or analysis.get("kicks", [])
...
def build_zoom_expression(
    ...
    """
    Expressão de zoom com base cíclica, section modulation, beat/bass/drop pulses.
    """
    beats     = analysis.get("beats", [])
    bass_hits = analysis.get("bass_hits") or analysis.get("kicks", [])
    drop_time = analysis.get("drop_time")
...
def _section_zoom_mod(sections: dict, fps: int) -> str:
    """Multiplicador de zoom por seção: intro sutil → buildup ramp → drop máximo."""
...
def build_remotion_audio_data(analysis: dict, rms: list | None = None) -> dict:
    """Monta JSON compatível com Composition.tsx do Remotion."""
